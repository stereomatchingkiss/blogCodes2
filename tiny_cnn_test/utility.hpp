                 ...
                 int y_padding = 0)
{
    ...
    if (x_padding || y_padding){
        int const w = input.cols + 2 * x_padding;
        int const h = input.rows + 2 * y_padding;
        int const area = w * h;
        ...
        for (int c = 0; c < input.channels(); ++c) {
            for (int y = 0; y < input.rows; ++y) {
                for (int x = 0; x < input.cols; ++x) {
                    img[c * area + (y + y_padding) * w + x + x_padding]
                            = scale_min + range *
        ...
        {
           img.emplace_back(scale_min + (scale_max - scale_min) * c / 255);
        });
    }
