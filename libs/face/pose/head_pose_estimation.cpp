...
{
    cv::Mat const camera_matrix = create_camera_matrix(img_size);
    ...
    return quaterniond_to_euler_angle(create_quaterniond(rotation_vector));
}
...
cv::Mat head_pose_estimation::create_camera_matrix(const cv::Size &img_size) const
{
    return create_camera_matrix(img_size.width, cv::Point2d(img_size.width/2, img_size.height/2));
}

head_pose_estimation::quaterniond head_pose_estimation::create_quaterniond(const cv::Mat &rotation_vector) const
{
...
head_pose_info head_pose_estimation::quaterniond_to_euler_angle(head_pose_estimation::quaterniond const &q) const
{
    ...
    if(result.pitch_ > 0){
        result.pitch_ = 180 - result.pitch_;
    }else if(result.pitch_ < 0){
        result.pitch_ = 180 + result.pitch_;
    }
