...
{
    cv::Mat map;
    get_projection_map_hue(input, model, map);

    return map;
}

void histProject::get_projection_map_hue(cv::Mat const &input, cv::Mat const &model, cv::Mat &output)
{
...
void histProject::get_projection_map_hue_sat(cv::Mat const &input, cv::Mat const &model, cv::Mat &output, int min_saturation)
{
    ...
    convert_to_hsv(model, model_hsv_);
    if(min_saturation > 0){
        saturation_mask_.create(model_hsv_.size(), model_hsv_.depth());
        mix_channels(model_hsv_, saturation_mask_, {1, 0});
        cv::threshold(saturation_mask_, saturation_mask_, min_saturation, 255, cv::THRESH_BINARY);
        calc_histogram<2>({model_hsv_}, model_hist_, {0, 1}, {180, 256}, {{ {0, 180}, {0, 256} }}, saturation_mask_);
    }else{
