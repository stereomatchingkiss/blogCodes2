...
#@torch.jit.script		
#def slice_layer(tensor: torch.Tensor, height: int, width: int):
#    return torch.tensor([3, 5], dtype=torch.int)#tensor[..., :height, :width]
...
class DexiNet(nn.Module):
    ...
    def slice(self, tensor, height, width):        
...
#        block_5_pre_dense_256 = self.pre_dense_6(block_5_add) # if error uncomment
        ...
        # use fixed value to avoid warning
        out_1 = self.up_block_1(block_1)[..., :400, :400]
        out_2 = self.up_block_2(block_2)[..., :400, :400]
        out_3 = self.up_block_3(block_3)[..., :400, :400]
        out_4 = self.up_block_4(block_4)[..., :400, :400]
        out_5 = self.up_block_5(block_5)[..., :400, :400]
        out_6 = self.up_block_6(block_6)[..., :400, :400]        
        
        # upsampling blocks
        #height, width = x.shape[-2:]
        #slice_shape = (height, width)
        		
        #print("out1 type is = ", type(out_1))
        #print("out1 shape = ", out_1.shape, ", height = ", height, ", width = ", width)        
        #out_1 = self.slice(self.up_block_1(block_1), height, width)
        #print("out1 shape = ", out_1.shape, type(out_1))
        #out_2 = self.slice(self.up_block_2(block_2), height, width)
        #print("out2 shape = ", out_2.shape)
        #out_3 = self.slice(self.up_block_3(block_3), height, width)
        #print("out3 shape = ", out_3.shape)
        #out_4 = self.slice(self.up_block_4(block_4), height, width)
        #print("out4 shape = ", out_4.shape)
        #out_5 = self.slice(self.up_block_5(block_5), height, width)
        #print("out4 shape = ", out_5.shape)
        #out_6 = self.slice(self.up_block_6(block_6), height, width)
        #print("out6 shape = ", out_6.shape)
